...
const getVerifiedLinks = (q) => {
  ...
  if (query.includes("pm") || query.includes("prime minister")) {
    return [
      { title: "Wikipedia", url: "https://en.wikipedia.org/wiki/List_of_current_heads_of_state_and_government" }
    ];
  }
...
const isDynamicFact = (t) => {
  const patterns = ["prime minister","president","news","stock","weather"];
  return patterns.some(p => t.includes(p));
};

const isEducation = (t) => {
  const patterns = ["explain","define","what is","science","math","physics"];
  return patterns.some(p => t.includes(p));
};
...
module.exports = async (req, res) => {
  ...
  const { message, history = {}, plan, user_id } = req.body || {};
  ...
  if (isDynamicFact(text)) {
    return res.json({
      reply: "Real-time info detected. Use trusted sources.",
  ...
  if (isEducation(text)) {
    const response = await fetch("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({
        model: "gpt-4o-mini",
        input: [
          { role: "system", content: "Explain step-by-step like a teacher." },
          ...cleanHistory,
